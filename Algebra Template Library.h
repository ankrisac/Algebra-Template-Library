...
/*
<!Initial Push: 21/12/2017 : 22:04 : GMT+4>
Algebraic Template Library>
	> Possible Future Additions :
		> Complex Number class (?)
		> Row echelon finding
		> Eigen values & vectors
		> Numerical & Symbolic intergrators & differentiators (???)
		> ...
*/
...
namespace atl{ 
	...
	namespace mtl {
		...
		template<typename T> std::string vector_to_string(const std::vector<T> &vec, const std::string sep = ",", 
		...
		public:
			...
			T& unsafe_get(const size_t i) {
				return m_elements[i];
			}
			const T& unsafe_get(const size_t i) const {
				return m_elements[i];
			}
			T& at(const size_t i) {
			...
			void resize(const size_t size, const T& val) {
				m_elements.resize(size, val);
				m_elements.shrink_to_fit();
			}
			size_t size() {
				return m_elements.size();
			}
			std::string str(std::string sep = ",", std::string begin = "[", std::string end = "]\n") {
		...
		template<typename T> inline matrix<T> operator+(matrix<T> l_mat, const matrix<T> &r_mat) {
			l_mat += r_mat;
			return l_mat;
		}
		template<typename T> inline matrix<T> operator-(matrix<T> l_mat, const matrix<T> &r_mat) {
		...
		template<typename T> inline matrix<T> operator*(matrix<T> l_mat, const T &r_val) {
			l_mat *= r_val;
			return l_mat;
		}
		template<typename T> inline matrix<T> operator*(const T &l_val, matrix<T> r_mat) {
			r_mat *= l_val;
			return r_mat;
		}
		...
		public:
			...
			template<size_t CR> static f_matrix<T, R, C> mult(const f_matrix<T, R, CR> &mat_1, const f_matrix<T, CR, C> &mat_2) {
		...
		template<typename T, size_t R, size_t C> std::ostream& operator<<(std::ostream &out, const f_matrix<T, R, C> &mat) {
			return out << mat.str();
		}
		template<typename T, size_t R, size_t C> inline f_matrix<T, R, C> operator+(f_matrix<T, R, C> l_mat, const f_matrix<T, R, C> &r_mat) {
			l_mat += r_mat;
			return l_mat;
		}
		template<typename T, size_t R, size_t C> inline f_matrix<T, R, C> operator-(f_matrix<T, R, C> l_mat, const f_matrix<T, R, C> &r_mat) {
		...
		template<typename T, size_t R, size_t CR, size_t C> inline f_matrix<T, R, C> operator*(const f_matrix<T, R, CR> &l_mat, const f_matrix<T, CR, C> &r_mat) {
			return f_matrix<T, R, C>::mult(l_mat, r_mat);
		}
		template<typename T, size_t R, size_t C> inline f_matrix<T, R, C> operator*(f_matrix<T, R, C> l_mat, const T &r_val) {
			l_mat *= r_val;
			return l_mat;
		}
		template<typename T, size_t R, size_t C> inline f_matrix<T, R, C> operator*(const T &l_val, f_matrix<T, R, C> r_mat) {
			r_mat *= l_val;
			return r_mat;
		}
	}
